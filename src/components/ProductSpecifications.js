...
const ProductSpecifications = () => {
    ...
    async (type) => {
      ...
      try {
        ...
        const response = await apiClient.post(
          ...
          type === "images"
            ? data
            : { [`${type.slice(0, -1)}_name`]: data.content },
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        if (type === "colors")
          setColors((prev) => [...prev, response.data.color]);
        if (type === "sizes") setSizes((prev) => [...prev, response.data.size]);
        setToast({
          type: "success",
          message: `${type.capitalize()} created successfully`,
        });
        setNewSpecForm((prev) => ({
          ...prev,
          [type]:
            type === "images" ? { pro_id: "", imageURL: "" } : { content: "" },
        }));
        setShowAddForm((prev) => ({ ...prev, [type]: false }));
      } catch (err) {
        setError(
          err.response?.data?.message || `Failed to create ${type.slice(0, -1)}`
        );
        setToast({
          type: "error",
          message:
  ...
  const updateSpecification = useCallback(async (type, id, data) => {
    ...
    try {
      ...
      if (type === "images") {
        setImages((prev) =>
          prev.map((item) => (item._id === id ? response.data.image : item))
        );
      }
      if (type === "colors") {
        setColors((prev) =>
          prev.map((item) => (item._id === id ? response.data.color : item))
        );
      }
      if (type === "sizes") {
        setSizes((prev) =>
          prev.map((item) => (item._id === id ? response.data.size : item))
        );
      }
      setToast({
        type: "success",
        message: `${type.capitalize()} updated successfully`,
      });
      setEditingSpecId(null);
      setEditFormData({});
    } catch (err) {
      setError(
        err.response?.data?.message || `Failed to update ${type.slice(0, -1)}`
      );
      setToast({
        type: "error",
        message:
  ...
  const deleteSpecification = useCallback(
    async (type, id) => {
      ...
      try {
        ...
        setToast({
          type: "success",
          message: `${type.capitalize()} deleted successfully`,
        });
        if (editingSpecId === id) setEditingSpecId(null);
      } catch (err) {
        setError(
          err.response?.data?.message || `Failed to delete ${type.slice(0, -1)}`
        );
        setToast({
          type: "error",
          message:
  ...
  if (isAuthLoading) {
    return (
      <div className="specifications-container">
        <div
          className="specifications-loading"
          role="status"
          aria-live="polite"
        >
  ...
  return (
    <div className="specifications-container">
      ...
      {toast && (
        <div
          className={`specifications-toast ${
            toast.type === "success"
              ? "specifications-toast-success"
              : "specifications-toast-error"
          }`}
          role="alert"
          aria-live="assertive"
        >
      ...
      {error && (
        <div
          className="specifications-error"
          role="alert"
          aria-live="assertive"
        >
        ...
        <div className="specifications-section">
          <div className="specifications-add-button-container">
            <button
              ...
              aria-label={
                showAddForm.images ? "Cancel adding image" : "Add new image"
              }
            >
            ...
            <div className="specifications-add-form">
              ...
              <div className="specifications-form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleNewFieldChange(e, "images", "imageURL")
                  }
                    ...
                    <tr key={image._id} className="specifications-table-row">
                      ...
                      <td>
                        {editingSpecId === image._id ? (
                          ...
                          image.pro_id?.pro_name ||
                          getProductName(image.pro_id) ||
                          "N/A"
                        )}
                      </td>
                      <td>
                        {editingSpecId === image._id ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleEditFieldChange(e, "imageURL")
                            }
                            className="specifications-form-input"
                            aria-label="Image URL"
                            required
                      ...
                      <td>
                        {editingSpecId === image._id ? (
                          <div className="specifications-action-buttons">
                            <button
                              onClick={() =>
                                handleUpdateSubmit("images", image._id)
                              }
                          ...
                          <div className="specifications-action-buttons">
                            ...
                            <button
                              onClick={() =>
                                deleteSpecification("images", image._id)
                              }
        ...
        <div className="specifications-section">
          <div className="specifications-add-button-container">
            <button
              ...
              className="specifications-add-button"
              aria-label={
                showAddForm.colors ? "Cancel adding color" : "Add new color"
              }
            >
                      ...
                      <td>
                        {editingSpecId === color._id ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleEditFieldChange(e, "content")
                            }
                            className="specifications-form-input"
                            aria-label="Color Name"
                            required
                      ...
                      <td>
                        {editingSpecId === color._id ? (
                          <div className="specifications-action-buttons">
                            <button
                              onClick={() =>
                                handleUpdateSubmit("colors", color._id)
                              }
                              className="specifications-update-button"
                          ...
                          <div className="specifications-action-buttons">
                            ...
                            <button
                              onClick={() =>
                                deleteSpecification("colors", color._id)
                              }
                              className="specifications-delete-button"
        ...
        <div className="specifications-section">
          <div className="specifications-add-button-container">
            <button
              ...
              className="specifications-add-button"
              aria-label={
                showAddForm.sizes ? "Cancel adding size" : "Add new size"
              }
            >
                      ...
                      <td>
                        {editingSpecId === size._id ? (
                          <div className="specifications-action-buttons">
                            <button
                              onClick={() =>
                                handleUpdateSubmit("sizes", size._id)
                              }
                              className="specifications-update-button"
                          ...
                          <div className="specifications-action-buttons">
                            ...
                            <button
                              onClick={() =>
                                deleteSpecification("sizes", size._id)
                              }
                              className="specifications-delete-button"
